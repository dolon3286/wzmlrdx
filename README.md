...
```bash
/mirror {video_link} -mkv ["mkvmerge -o mltb.SUB.EN.mkv mltb.video --language 0:eng https://example.com/subtitle.srt"]
```
...
```bash
/mirror {video_link} -mkv ["mkvmerge -o mltb.SUB.EN.mkv mltb.video --language 0:eng /usr/src/app/subs/movie.srt"]
```

```bash
(Reply to subtitle file) /mirror{suffix} {video_link} -mkvsub
```
(quick mode: auto-runs `mkvmerge`, replaces original source with merged file before upload, and auto-cleans temporary subtitle file; suffix example: /mirror8 or /m8
