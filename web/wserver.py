...
def parse_tg_session_token(token: str):
    try:
        ...
        if not bot_token:
            return None
        expected = hmac_new(bot_token.encode(), payload.encode(), sha256).hexdigest()
...
"""
        ...
        if not bot_token or not db_url:
            return HTMLResponse(
                "<h3>Missing BOT_TOKEN or DATABASE_URL in environment.</h3>",
                status_code=500,
            )
        db_id = bot_token.split(":", 1)[0]
        conn = AsyncIOMotorClient(db_url, server_api=ServerApi("1"))
        try:
            db = conn.wzmlx
